...
- 用户可以把 Obsidian Markdown 文件的 Frontmatter 当作元数据入口来过滤查询记忆。
- 用户可以控制哪些 Agent 产出的内容进入正式记忆。
- 用户可以在 Obsidian 中审计和修改正式记忆。

## 功能描述

### Story 1：使用 Frontmatter 过滤查询记忆

作为 Obsidian 用户，我希望每条记忆的元数据保存在 Markdown Frontmatter 中，并能按这些字段过滤查询，以便沿用 Obsidian 的文件和属性工作流。

验收标准：

- Given Vault 中存在带 YAML Frontmatter 的 Markdown 记忆文件
  When 用户按 status、type、scope、project、tags 或 source_agent 过滤查询
  Then 系统只返回 Frontmatter 字段匹配的 Markdown 文件
- Given 查询结果包含多条记忆
  When 系统返回结果
  Then 每条结果包含文件路径、标题、Frontmatter 元数据和正文摘要
- Given Markdown 文件缺少必需 Frontmatter 字段 status、type、scope、source_agent、created_at 或 evidence
  When 系统构建 active 记忆查询结果
  Then 该文件不进入 active 查询结果，并返回可定位到文件的校验错误
...
- Given Vault 中存在 status 为 active 且 project 匹配当前任务的 profile memory、project memory、episode 和 decision
...
- Given Agent 完成一次项目任务
  ...
  Then Vault 中新增一条 Markdown episode 记录，且 Frontmatter status 为 active、type 为 episode
...
- Given Agent 识别到长期记忆候选
  ...
  Then Vault 的 Inbox 中新增一条 Markdown proposal 记录，且 Frontmatter status 为 proposed
- Given proposal 内容缺少 evidence、type、scope、source_agent、confidence 或 created_at
  When 系统接收该 proposal
  Then 系统拒绝写入，并返回可读错误信息
- Given proposal 类型为 preference、fact、identity 或 sensitive
  When 系统完成接收
  Then proposal 的 Frontmatter requires_approval 为 true
...
- Given Inbox 中存在 proposed 记录
  When 用户列出待审核项
  Then 系统按 Frontmatter created_at 倒序返回全部 status 为 proposed 的记录
- Given 用户批准一条 proposed 记录
  When 系统提交该记录
  Then 正式记忆目录新增 Markdown active 记录，原 proposal 的 Frontmatter status 标记为 committed
- Given 用户拒绝一条 proposed 记录
  When 系统保存拒绝结果
  Then proposal 的 Frontmatter status 变为 rejected，正式记忆目录不新增记录
...
- Given 项目存在 Frontmatter project 匹配的 active task、blocked task 和 done task
  When Agent 请求项目状态
  Then 系统返回项目背景、活动任务、阻塞任务、已完成任务和最近决策
- Given Agent 更新项目任务状态
  When 更新内容包含项目名、任务名、状态、来源 Agent
  Then Vault 中的 Markdown 项目状态记录被更新，并在 Frontmatter 保留更新时间
...
- Given 用户在空目录执行初始化
  When 初始化完成
  Then Vault 目录结构、系统配置记录、Inbox 目录和 Frontmatter schema 说明存在
- Given Vault 中已有 active 记忆
  When 用户执行带 Frontmatter 过滤条件的检索命令
  Then 命令输出匹配的上下文包，并包含 sources 字段
- Given 用户执行重建索引命令
  When 命令完成
  Then 检索结果与 Vault 中 Frontmatter status 为 active 的 Markdown 记录保持一致
...
- Markdown 文件作为记忆、事件、候选和项目状态的存储单元。
- YAML Frontmatter schema 校验。
- 按 Frontmatter status、type、scope、project、tags、source_agent 过滤查询。
- 上下文包读取。
- episode 自动写入。
...
- 审计约束：所有 active 长期记忆必须在 Frontmatter 中包含 source_agent、created_at、status、type、scope、evidence。
